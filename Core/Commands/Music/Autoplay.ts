  ...
  SlashCommandGen: new SlashCommandBuilder()
    ...
    Execute: async (interaction) => {
        ...
        try {
            ...
            const ErrorHandler = new StatementBuilder()
                .addNullishStatement(Member.voice.channel, `${Emojis.Error} ${General.NoChannel}`, 
                (Channel, ErrorHandler) => ErrorHandler
                    .addNormalStatement(Channel, bold(`${Emojis.Error} ${General.NoQueue}`), (Input) => !Queue)
                    .addNormalStatement(Channel, bold(`${Emojis.Error} ${General.DifferentChannel}`), (Input) => {
