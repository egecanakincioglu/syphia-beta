...
export default new CommandGen({
  SlashCommandGen: new SlashCommandBuilder().setName('ping').setDescription(Cmd.Description),
  Execute: async (interaction: ChatInputCommandInteraction) => {
    const AyumiPing = await getEmoji('AyumiPing');
    const KlausTimeout = await getEmoji('KlausTimeout');

    const startTimestamp = Date.now();
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId('ping_button').setLabel(Cmd.ButtonLabel).setStyle(ButtonStyle.Primary));

    const { PingEmbed, PingDescription } = await PingInterface.ReplacePing();
    const NewPingEmbed = PingEmbed.setThumbnail(interaction.guild.iconURL()).setFooter({
      text: `${interaction.user.displayName} ${Cmd.Footer}`,
