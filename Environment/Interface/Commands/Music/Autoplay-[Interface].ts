...
export class AutoplayInterface {
    ...
    public static async ReplaceAutoplay() {
        ...
        const DescriptionMessages: string[] = [
            `${SyphiaDecorate} ${Cmd.Messages[0]}`,
            `${SyphiaMessage} ${Cmd.Messages[1]}`,
        ];

        return {
            AutoplayEmbed: this.AutoplayEmbed
                .setAuthor({ name: Cmd.Title, iconURL: await getEmojiLink('SyphiaStar') })
                .setDescription(DescriptionMessages.join("\n\n")),
